...
export default function ConsumerLogin() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Prepare payload: For login, identifier goes in 'email' field in backend logic
      ...
      if (res.data.success) {
        if (!isLogin) {
          ...
          // Clear password but keep identifier for easy login
          setFormData(prev => ({ ...prev, password: '' }));
        } else {
          // 3. Robust Session Storage
          localStorage.setItem('token', res.data.token);
          localStorage.setItem('role', 'consumer');
          localStorage.setItem('name', res.data.name || 'Consumer');
          localStorage.setItem('userId', res.data.consumer_id);
          localStorage.setItem('mobile', res.data.mobile || '');
          localStorage.setItem('userLocation', res.data.location || 'Guntur');
      ...
      if (err.code === 'ERR_NETWORK') {
        ...
        setError('Incorrect credentials. Please verify your email/mobile and password.');
      } else {
